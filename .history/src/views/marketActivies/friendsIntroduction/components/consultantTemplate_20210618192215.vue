...
<el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="80px" label-position="left">
    ...
    <el-form-item label="选择落地页(请搜索活动名称)：" prop="landingPage" label-width="500" class="replace-auto-style">
        <el-select v-model="ruleForm.landingPage" filterable remote reserve-keyword no-data-text="无匹配结果" :loading-text="loadingText" placeholder="" @change="changeLandPage" @focus="getLandingPages" :remote-method="getLandingPages" :loading="landPageLoading">
            <el-option v-for="item in landPageOptions" :key="item.id" :label="item.name" :value="item.name">
            </el-option>
        </el-select>
    </el-form-item>
    ...
    data() {
        ...
        return {
            ...
            rules: {
                ...
                prize1: [{
                    validator: prizeValidate,
                    message: '至少填1个奖品',
                    trigger: 'blur'
                }],
                prize2: [{
                    validator: prizeValidateOther,
                    trigger: 'blur'
                }],
                prize3: [{
                    validator: prizeValidateOther,
